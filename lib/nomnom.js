ArgParser = function(options, parserOpts) {  
  ...
  this.print = parserOpts.printFunc;
...
exports.parseArgs = function(opts, parserOpts) {
  /* all the node-specific stuff */
  parserOpts = parserOpts || {};
  parserOpts.script = parserOpts.script || process.argv[0] + " "
        + require('path').basename(process.argv[1]);
  parserOpts.printFunc = parserOpts.printFunc || function(str) {
    require("sys").puts(str);
    process.exit(0);
  };
  var argv = parserOpts.argv || process.argv.slice(2);

  return (new ArgParser(opts, parserOpts)).parse(argv);
};